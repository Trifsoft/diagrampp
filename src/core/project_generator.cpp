...
void ProjectGenerator::remove_existing_project(const fs::path& path){
    fs::remove_all(path); // recursively delete content in dir on path and dir on path itself
}

ProjectGenerator::GenerationStatusCode ProjectGenerator::determine_error_code(const std::error_code& ec)
{
...
const std::unordered_set<SharedNodePtr> ProjectGenerator::retrieve_diagram_nodes(const graph_type& diagram)
{
...
bool ProjectGenerator::should_generate_cpp_file(SharedNodePtr node)
{
    return !node->definition().isEmpty();
}

fs::path& ProjectGenerator::append_n_times(fs::path& base_dir, std::vector<std::string>& dirs)
{
    for(auto& d : dirs){
        base_dir.append(d);
    }
...
const std::string ProjectGenerator::format_file_name(const std::string& name,
                                                     const ProjectGenerator::FileNameNotation notation)
{
...
ProjectGenerator::GenerationStatusCode ProjectGenerator::generate_cmake(const fs::path& path,
...
ProjectGenerator::GenerationStatusCode ProjectGenerator::generate_dir_hierarchy(const fs::path& root_path,
                                                                                std::error_code& ec)
{
...
ProjectGenerator::GenerationStatusCode ProjectGenerator::generate_files(const graph_type& diagram,
...
ProjectGenerator::GenerationStatusCode ProjectGenerator::generate(const graph_type& diagram,
                                                                  const QString& path,
                                                                  QString& project_dir_name,
                                                                  const ProjectGenerator::FileNameNotation notation,
                                                                  const ProjectGenerator::ReplaceToggle replace_mode){
    namespace fs = std::filesystem;
    ...
    project_dir_name.erase(std::remove(project_dir_name.begin(), project_dir_name.end(), '\\'), project_dir_name.end());
    fs::path root_path = fs::path(path.toStdString());

    if(!fs::exists(root_path) || !fs::is_directory(root_path)){
        return ProjectGenerator::GenerationStatusCode::NO_SUCH_DIR;
    }

    root_path.append(project_dir_name.toStdString());
